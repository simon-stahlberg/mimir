                                                                   ...
                                                                   const std::vector<size_t>& derived_ground_atoms_order)
{
    ...
    {
        const bool positive_condition = (is_fluent) ? contains(element->get_conjunctive_condition().template get_positive_precondition<Fluent>(), atom_index) :
                                                      contains(element->get_conjunctive_condition().template get_positive_precondition<Derived>(), atom_index);
        const bool negative_condition = (is_fluent) ? contains(element->get_conjunctive_condition().template get_negative_precondition<Fluent>(), atom_index) :
                                                      contains(element->get_conjunctive_condition().template get_negative_precondition<Derived>(), atom_index);
