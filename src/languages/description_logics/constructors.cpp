...
{
    // Fetch data
    auto& bitset = boost::hana::at_key(context.get_builders(), boost::hana::type<Concept> {}).get_data();
...
void RoleUniversalImpl::evaluate_impl(EvaluationContext& context) const
{
    // Fetch data
    const auto num_objects = context.get_problem()->get_problem_and_domain_objects().size();
    auto& bitsets = boost::hana::at_key(context.get_builders(), boost::hana::type<Role> {}).get_data();
...
{
    // Fetch data
    auto& boolean = boost::hana::at_key(context.get_builders(), boost::hana::type<Boolean> {}).get_data();
    boolean = false;
...
void NumericalCountImpl<D>::evaluate_impl(EvaluationContext& context) const
{
    ...
    // Fetch data
    auto& numerical = boost::hana::at_key(context.get_builders(), boost::hana::type<Numerical> {}).get_data();
