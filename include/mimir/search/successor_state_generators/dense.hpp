...
private:
    ...
    [[nodiscard]] DenseState get_or_create_successor_state_impl(const DenseState state, const DenseGroundAction action)
    {
        ...
        const auto positive_static_initial_atoms = state.get_problem()->get_static_initial_positive_atoms_bitset();
        const auto negative_static_initial_atoms = state.get_problem()->get_static_initial_negative_atoms_bitset();

        /* Conditional effects */
        const auto num_conditional_effects = action.get_conditional_effects().size();
        for (size_t i = 0; i < num_conditional_effects; ++i)
        {
            assert(positive_static_initial_atoms.is_superseteq(action.get_conditional_positive_static_precondition_bitsets()[i]));
            ...
            if (state.get_atoms_bitset().is_superseteq(action.get_conditional_positive_precondition_bitsets()[i])
                ...
                && positive_static_initial_atoms.are_disjoint(action.get_conditional_negative_static_precondition_bitsets()[i]))
            {
