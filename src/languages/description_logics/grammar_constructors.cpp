...
{
    return std::visit([](const auto& arg) -> size_t { return HashCombiner()(&arg.get()); }, m_choice);
}
...
size_t DerivationRule<D>::hash() const
{
    ...
    std::for_each(m_choices.begin(), m_choices.end(), [&seed](const auto& choice) { HashCombiner()(seed, &choice.get()); });
    return seed;
}
...
size_t ConceptAnd::hash() const { return HashCombiner()(&m_concept_left, &m_concept_right); }
...
size_t RoleAnd::hash() const { return HashCombiner()(&m_role_left, &m_role_right); }
