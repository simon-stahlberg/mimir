...
bool DomainImpl::is_structurally_equivalent_to_impl(const DomainImpl& other) const {
    return (m_name == other.m_name)
        ...
        && (loki::get_sorted_vector(m_types) == loki::get_sorted_vector(other.m_types))
...
namespace std {
    bool less<mimir::Domain>::operator()(
        const mimir::Domain& left_domain,
        const mimir::Domain& right_domain) const {
        return *left_domain < *right_domain;
    }

    std::size_t hash<mimir::DomainImpl>::operator()(const mimir::DomainImpl& domain) const {
        return domain.hash();
    }
}
