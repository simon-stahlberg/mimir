...
namespace mimir 
{
    ...
    size_t FunctionSkeletonImpl::hash_impl() const {
        return loki::hash_combine(m_name, loki::hash_container(m_parameters), m_type);
    }

    void FunctionSkeletonImpl::str_impl(std::ostringstream& out, const loki::FormattingOptions& options) const {
...
namespace std 
{
    bool less<mimir::FunctionSkeleton>::operator()(
        const mimir::FunctionSkeleton& left_function,
        const mimir::FunctionSkeleton& right_function) const {
        return *left_function < *right_function;
    }

    std::size_t hash<mimir::FunctionSkeletonImpl>::operator()(const mimir::FunctionSkeletonImpl& function) const {
