...
TEST(MimirTests, GroundedAstarTest) {
    ...
    auto grounded_astar = Algorithm<AStarAlgorithmTag<GroundedTag, ZeroHeuristicTag>>(problem);
...
TEST(MimirTests, LiftedAstarTest) {
    ...
    auto problem = static_cast<Problem>(nullptr);
    auto lifted_astar = Algorithm<AStarAlgorithmTag<LiftedTag, ZeroHeuristicTag>>(problem);
    GroundActionList plan;
