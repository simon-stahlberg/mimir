...
/// @brief `DenseStateImpl` encapsulates the fluent and derived atoms of a planning state.
...
template<>
struct cista::storage::DerefStdHasher<mimir::DenseStateImpl>
{
    size_t operator()(const mimir::DenseStateImpl* ptr) const;
};
template<>
struct cista::storage::DerefStdEqualTo<mimir::DenseStateImpl>
{
    bool operator()(const mimir::DenseStateImpl* lhs, const mimir::DenseStateImpl* rhs) const;
};

namespace mimir
{

using DenseStateImplSet = cista::storage::UnorderedSet<DenseStateImpl>;
