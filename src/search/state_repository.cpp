                                                                          ...
                                                                          const FlatDoubleList& fluent_numeric_variables)
{
    ...
    state_numeric_variables = valla::insert(dense_fluent_numeric_variables, index_tree_table, double_leaf_table);

    assert(std::equal(dense_fluent_numeric_variables.begin(),
                      dense_fluent_numeric_variables.end(),
                      valla::begin(state_numeric_variables, index_tree_table, double_leaf_table)));
    ...
    state_fluent_atoms_slot = valla::insert(dense_fluent_atoms, index_tree_table);
    ...
    /* 3. Apply axioms to construct extended state. */
    {
        if (!m_axiom_evaluator->get_problem()->get_problem_and_domain_axioms().empty())
        {
            ...
            state_derived_atoms_slot = valla::insert(dense_derived_atoms, index_tree_table);
