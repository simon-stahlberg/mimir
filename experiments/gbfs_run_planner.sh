...
# Run planner
"$planner_exe" "-D" "$domain_file" "-P" "$problem_file" "-O" "$plan_file" "-A" "$gbfs_mode" "-W0" "$weight_preferred_queue" "-W1" "$weight_standard_queue" "-H" "$heuristic_type" "-M" "$search_mode" "-L" "$lifted_mode" "-S" "$symmetry_pruning_mode"
