...
struct serialization_context
{
    ...
    static bool compare(std::pair<void const*, vector_range> const& a, std::pair<void const*, vector_range> const& b) noexcept { return a.first < b.first; }

    offset_t write(void const* ptr, std::size_t const size, std::size_t const alignment = 0) { return t_.write(ptr, size, alignment); }
    ...
    std::optional<offset_t> resolve_vector_range_ptr(Ptr ptr)
    {
        ...
        return pred->second.contains(pred->first, ptr) ? std::make_optional(pred->second.offset_of(pred->first, ptr)) : std::nullopt;
    }

    std::uint64_t checksum(offset_t const from) const noexcept { return t_.checksum(from); }
...
void serialize(Ctx& c, basic_vector<T, Ptr, Indexed, TemplateSizeType> const* origin, offset_t const pos)
{
    ...
    auto const start = origin->empty() ? NULLPTR_OFFSET : c.write(static_cast<T const*>(origin->el_), size, std::alignment_of_v<T>);

    c.write(pos + cista_member_offset(Type, el_), convert_endian<Ctx::MODE>(start == NULLPTR_OFFSET ? start : start - cista_member_offset(Type, el_) - pos));
...
void serialize(Ctx& c, generic_cstring<Ptr> const* origin, offset_t const pos)
{
    ...
    c.write(pos + cista_member_offset(Type, h_.ptr_), convert_endian<Ctx::MODE>(start - cista_member_offset(Type, h_.ptr_) - pos));
    c.write(pos + cista_member_offset(Type, h_.size_), convert_endian<Ctx::MODE>(origin->h_.size_));
    c.write(pos + cista_member_offset(Type, h_.self_allocated_), false);
...
void serialize(Ctx& c, basic_unique_ptr<T, Ptr> const* origin, offset_t const pos)
{
    ...
    auto const start = origin->el_ == nullptr ? NULLPTR_OFFSET : c.write(origin->el_, serialized_size<T>(), std::alignment_of_v<T>);
...
template<typename Ctx, typename T, template<typename> typename Ptr, typename GetKey, typename GetValue, typename Hash, typename Eq>
void serialize(Ctx& c, hash_storage<T, Ptr, GetKey, GetValue, Hash, Eq> const* origin, offset_t const pos)
{
    ...
    auto const start =
        origin->entries_ == nullptr ?
            ...
            c.write(origin->entries_,
                    static_cast<std::size_t>(origin->capacity_ * serialized_size<T>() + (origin->capacity_ + 1 + Type::WIDTH) * sizeof(typename Type::ctrl_t)),
    ...
    {
        auto i = 0u;
        for (auto it = start; it != start + static_cast<offset_t>(origin->capacity_ * serialized_size<T>()); it += serialized_size<T>(), ++i)
        {
...
template<typename Ctx, typename T>
void serialize(Ctx& c, basic_external_ptr<T> const* origin, offset_t const pos)
{
    using Type = basic_external_ptr<T>;
    serialize(c, &origin->el_, pos + cista_member_offset(Type, el_));
}
...
{
    ::cista::apply([&](auto&&... args) { (serialize(c, &args, offset + (reinterpret_cast<intptr_t>(&args) - reinterpret_cast<intptr_t>(origin))), ...); },
                   *origin);
}
...
struct deserialization_context
{
    ...
    void deserialize_ptr(Ptr** ptr) const
    {
        ...
        *ptr = offset == NULLPTR_OFFSET ? nullptr : reinterpret_cast<Ptr*>(checked_addition(reinterpret_cast<offset_t>(ptr), offset));
    }
...
void check(std::uint8_t const* const from, std::uint8_t const* const to)
{
    ...
    if constexpr ((Mode & mode::WITH_INTEGRITY) == mode::WITH_INTEGRITY)
    {
        verify(convert_endian<Mode>(*reinterpret_cast<std::uint64_t const*>(from + integrity_start(Mode)))
                   == hash(std::string_view { reinterpret_cast<char const*>(from + data_start(Mode)), static_cast<std::size_t>(to - from - data_start(Mode)) }),
               "invalid checksum");
    }
}
...
template<typename Ctx, typename T, typename SizeType, template<typename> typename Vec, std::size_t Log2MaxEntriesPerBucket, typename Fn>
...
void check_state(Ctx const& c, hash_storage<T, Ptr, GetKey, GetValue, Hash, Eq>* el)
{
    ...
    c.require((el->entries_ == nullptr) == (el->capacity_ == 0U && el->size_ == 0U), "hash storage: entries=null <=> size=capacity=0");
    ...
    c.require(std::all_of(ptr_cast(el->ctrl_),
                          ...
                          { return Type::is_empty(ctrl) || Type::is_deleted(ctrl) || Type::is_full(ctrl) || ctrl == Type::ctrl_t::END; }),
    ...
    c.require(std::min(Type::capacity_to_growth(el->capacity_) - el->size_, total_growth_left) <= el->growth_left_, "hash storage: growth left");
}

template<typename Ctx, typename T, template<typename> typename Ptr, typename GetKey, typename GetValue, typename Hash, typename Eq, typename Fn>
...
T copy_from_potentially_unaligned(std::string_view buf)
{
    struct aligned
    {
        explicit aligned(std::string_view buf) : mem_ { static_cast<std::uint8_t*>(CISTA_ALIGNED_ALLOC(sizeof(max_align_t), buf.size())) }
        {
