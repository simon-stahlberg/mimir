...
namespace std {
    bool less<mimir::Problem>::operator()(
        const mimir::Problem& left_problem,
        const mimir::Problem& right_problem) const {
        return *left_problem < *right_problem;
    }

    std::size_t hash<mimir::ProblemImpl>::operator()(const mimir::ProblemImpl& problem) const {
        return problem.hash();
    }
}
