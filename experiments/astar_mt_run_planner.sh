...
if [[ $# != 12 ]]; then
    echo "usage: $(basename "$0") <planner_file> <domain_file> <problem_file> <plan_file> <heuristic_type:0> <enable_grounded:0/1> <enable_debug:0/1> <mt-enable_dump_dot_file:bool> <mt-max_num_nodes:int> <mt-split_strategy:int> <mt-split_metric:int> <mt-opt_dir:int>" 1>&2
    exit 2
fi
...
# Run planner
"$planner_exe" "$domain_file" "$problem_file" "$plan_file" "$flag1" "$flag2" "$flag3" "$flag4" "$flag5" "$flag6" "$flag7" "$flag8"
