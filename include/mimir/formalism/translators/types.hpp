...
private:
    ...
    GroundLiteralList translate_typed_object_to_ground_literals(const ObjectImpl& object)
    {
        ...
        for (const auto& type : types)
        {
            auto additional_literal = this->m_pddl_factories.ground_literals.template get_or_create<GroundLiteralImpl>(
                false,
                this->m_pddl_factories.ground_atoms.template get_or_create<GroundAtomImpl>(translate_type_to_predicate(*type),
    ...
    ConditionList translate_typed_parameter_to_condition_literals(const ParameterImpl& parameter)
    {
        ...
        for (const auto& type : types)
        {
            auto condition = this->m_pddl_factories.conditions.template get_or_create<ConditionLiteralImpl>(
                this->m_pddl_factories.literals.template get_or_create<LiteralImpl>(
                    false,
     ...
     * Translate
     */
    Object translate_impl(const ObjectImpl& object) { return translate_typed_object_to_untyped_object(object); }

    Parameter translate_impl(const ParameterImpl& parameter) { return translate_typed_parameter_to_untyped_parameter(parameter); }
    ...
    Action translate_impl(const ActionImpl& action)
    {
        ...
        auto translated_condition = conditions.empty() ?
                                        std::nullopt :
                                        std::optional<Condition>(this->m_pddl_factories.conditions.template get_or_create<ConditionAndImpl>(conditions));
    ...
    Problem run_impl(const ProblemImpl& problem) { return self().translate(problem); }

public:
    explicit TypeTranslator(PDDLFactories& pddl_factories) : BaseTranslator<TypeTranslator>(pddl_factories) {}
};
