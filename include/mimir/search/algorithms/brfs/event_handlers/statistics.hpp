...
inline std::ostream& operator<<(std::ostream& os, const Statistics& statistics)
{
    os << "[BrFS] Search time: " << statistics.get_search_time_ms().count() << "ms" << "\n"
       ...
       << "[BrFS] Number of bytes for states: " << statistics.get_num_bytes_for_states() << "\n"
       << "[BrFS] Number of bytes per states: " << divide_or_zero(statistics.get_num_bytes_for_states(), statistics.get_num_states()) << "\n"
