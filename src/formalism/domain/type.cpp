...
namespace mimir 
{
    ...
    size_t TypeImpl::hash_impl() const {
        return loki::hash_combine(m_name, loki::hash_container(loki::get_sorted_vector(m_bases)));
    }

    void TypeImpl::str_impl(std::ostringstream& out, const loki::FormattingOptions& /*options*/) const {
        out << m_name;
    }
...
namespace std 
{
    bool less<mimir::Type>::operator()(
        const mimir::Type& left_type,
        const mimir::Type& right_type) const {
        return *left_type < *right_type;
    }

    std::size_t hash<mimir::TypeImpl>::operator()(const mimir::TypeImpl& type) const {
        return type.hash();
    }
}
