...
public:
    ...
    /// @brief React on starting a search.
    virtual void on_end_arity_search(const BrFSAlgorithmStatistics& algorithm_statistics) = 0;
...
public:
    ...
    void on_end_arity_search(const BrFSAlgorithmStatistics& algorithm_statistics) override
    {
        m_statistics.push_back_algorithm_statistics(algorithm_statistics);

        if (!m_quiet)
        {
            self().on_end_arity_search_impl(algorithm_statistics);
        }
    }
