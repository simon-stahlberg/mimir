...
TEST(MimirTests, SearchStateBuilderTest)
{
    ...
    EXPECT_EQ(builder.get_buffer_writer().get_buffer().size(), 11);
...
TEST(MimirTests, SearchStateSetBuilderReuseTest)
{
    ...
    EXPECT_EQ(builder.get_buffer_writer().get_buffer().size(), 13);
    auto s1_state = *states.insert(builder).first;

    EXPECT_EQ(states.size(), 2);

    EXPECT_EQ(s1_state.get_buffer() - s0_state.get_buffer(), 11);
