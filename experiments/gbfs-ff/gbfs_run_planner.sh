...
if [[ $# != 8 ]]; then
    echo "usage: $(basename "$0") <planner_file> <domain_file> <problem_file> <plan_file> <eager:0/1> <heuristic_type:0> <enable_grounded:0/1> <enable_debug:0/1>" 1>&2
    exit 2
fi
...
# Run planner
"$planner_exe" "$domain_file" "$problem_file" "$plan_file" "$flag1" "$flag2" "$flag3" "$flag4"
